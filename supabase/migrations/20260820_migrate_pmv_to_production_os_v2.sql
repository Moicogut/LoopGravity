-- ==============================================================================
-- MIGRACIÓN DE TRANSICIÓN V2: PMV EXISTENTE A LOOPGRAVITY PRODUCTION OS (FASES 1, 2 Y 3)
-- Archivo: supabase/migrations/20260820_migrate_pmv_to_production_os_v2.sql
-- Repositorio: https://github.com/Moicogut/LoopGravity
-- Proyecto Supabase: https://bsftifcgyuaubmachzvi.supabase.co
-- ==============================================================================
-- AUDITORÍA Y CORRECCIONES APLICADAS EN V2:
-- 1. RLS explícito y granular para public.users y public.organization_members.
-- 2. Backfill 100% defensivo de asset_catalog mediante to_jsonb() para evitar errores
--    por columnas inexistentes (created_at, updated_at, role, gender).
-- 3. Inclusión exhaustiva de políticas RLS con USING y WITH CHECK en las 24 entidades.
-- 4. Preservación absoluta de las 5 tablas PMV y compatibilidad de tipos TEXT.
-- 5. Cero comandos DROP TABLE, TRUNCATE ni DELETE.
-- ==============================================================================
...
-- ==============================================================================
-- PASO 2: ORGANIZACIONES, USUARIOS Y MEMBRESÍAS
-- ==============================================================================
...
-- Sincronizar tenants existentes hacia organizations (idempotente y no destructivo)
DO $$
BEGIN
...
-- Tabla de Membresías Organizacionales
...
-- ==============================================================================
-- PASO 3: EVOLUCIÓN ADITIVA DE LA TABLA PROJECTS
-- ==============================================================================
...
-- ------------------------------------------------------------------------------
-- BACKFILL DEFENSIVO (CORRECCIÓN 2): Uso de to_jsonb() para tolerancia absoluta a columnas
-- ------------------------------------------------------------------------------
DO $$
DECLARE
  cat_row RECORD;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'asset_catalog') THEN
    FOR cat_row IN SELECT * FROM public.asset_catalog LOOP
      cat_json := to_jsonb(cat_row);
      
      cat_id := COALESCE(cat_json ->> 'id', gen_random_uuid()::text);
      cat_tenant_id := COALESCE(cat_json ->> 'tenant_id', 'unknown_tenant');
      cat_name := COALESCE(cat_json ->> 'name', 'Asset Sin Nombre');
      cat_description := cat_json ->> 'description';
      cat_image_url := COALESCE(cat_json ->> 'image_url', 'https://storage.loopgravity.io/assets/default.png');
      raw_category := COALESCE(cat_json ->> 'category', 'character');
      
      mapped_type := CASE 
        WHEN raw_category IN ('actors', 'character') THEN 'character'
        WHEN raw_category IN ('products', 'product') THEN 'product'
        WHEN raw_category IN ('environments', 'environment') THEN 'environment'
        WHEN raw_category IN ('outfit') THEN 'outfit'
        WHEN raw_category IN ('logo') THEN 'logo'
        ELSE 'character'
      END;

      -- Extraer atributos opcionales defensivamente
      cat_attributes := '{}'::jsonb;
      IF cat_json ? 'role' AND (cat_json ->> 'role') IS NOT NULL THEN
        cat_attributes := jsonb_set(cat_attributes, '{role}', cat_json -> 'role');
      END IF;
      IF cat_json ? 'gender' AND (cat_json ->> 'gender') IS NOT NULL THEN
        cat_attributes := jsonb_set(cat_attributes, '{gender}', cat_json -> 'gender');
      END IF;

      -- Extraer created_at si existe
      IF cat_json ? 'created_at' AND (cat_json ->> 'created_at') IS NOT NULL THEN
        BEGIN
          cat_created_at := (cat_json ->> 'created_at')::timestamptz;
      ...
      WHERE tenant_id = cat_tenant_id AND legacy_catalog_id = cat_id;
      ...
      IF new_asset_id IS NULL THEN
        ...
        VALUES (
          ...
          cat_image_url,
          cat_attributes,
          'Migración defensiva desde catálogo PMV',
          cat_created_at
        );
      END IF;
    END LOOP;
...
-- Provider Credentials (Solo metadatos de configuración)
...
-- ==============================================================================
-- PASO 7: ÍNDICES DE ALTO RENDIMIENTO
-- ==============================================================================
...
-- ==============================================================================
-- PASO 8: HABILITACIÓN DE RLS EN EL 100% DE LAS NUEVAS ENTIDADES
-- ==============================================================================
...
-- ==============================================================================
-- PASO 9: POLÍTICAS RLS ESTRICTAS (CORRECCIÓN 1 Y CORRECCIÓN 4)
-- ==============================================================================
DO $$
BEGIN
  -- 1. Organizations
  ...
  -- 2. Users (Un usuario autenticado solo puede leer y actualizar su propio perfil)
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'users' AND policyname = 'user_self_access_select') THEN
    CREATE POLICY user_self_access_select ON public.users 
      FOR SELECT USING (auth.uid() = id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'users' AND policyname = 'user_self_access_update') THEN
    CREATE POLICY user_self_access_update ON public.users 
      FOR UPDATE USING (auth.uid() = id) WITH CHECK (auth.uid() = id);
  END IF;

  -- 3. Organization Members (Consulta solo miembros de organizaciones del usuario autenticado; mutaciones por backend)
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'organization_members' AND policyname = 'org_member_tenant_select') THEN
    CREATE POLICY org_member_tenant_select ON public.organization_members 
      FOR SELECT USING (
        user_id = auth.uid() 
        OR organization_id = public.current_tenant_id()
      );
  END IF;
